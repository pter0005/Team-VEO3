...
export default function CurriculumSection() {
  return (
    <section id="como-funciona" className="py-16 md:py-24 bg-card scroll-mt-20">
      <div className="container mx-auto max-w-7xl px-4 md:px-6">
        ...
        <Card className="bg-background shadow-xl max-w-3xl mx-auto border-primary/30">
          <CardContent className="p-6 md:p-8">
            <ul className="space-y-4">
              {modules.map((module, index) => (
                <li key={index} className="flex items-start space-x-3">
                  <CheckCircle2 className="h-6 w-6 text-accent flex-shrink-0 mt-0.5" />
                  <span className="text-base md:text-lg text-foreground/90">{module}</span>
                </li>
              ))}
            </ul>
          </CardContent>
